...
class GameEngineV1Spec extends munit.CatsEffectSuite with ScalaCheckEffectSuite {
  ...
  test("game Engine should process simple MoveUp successfully".tag(Scenarios.Simple)) {
    GameEngine.v1[IO](
      ...
      } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 2, y = 4), history = List(PlayerInput.MoveUp)))
    }
  }
  ...
  test("game Engine should process simple MoveDown successfully".tag(Scenarios.Simple)) {
    GameEngine.v1[IO](
      ...
      for {
        ...
        playerData <- engine.getGameData.map(_.player)
      } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 2, y = 2), history = List(PlayerInput.MoveDown)))
    }
  }
  ...
  test("game Engine should process simple MoveRight successfully".tag(Scenarios.Simple)) {
    GameEngine.v1[IO](
      ...
      for {
        ...
        playerData <- engine.getGameData.map(_.player)
      } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 3, y = 3), history = List(PlayerInput.MoveRight)))
    }
  }
    ...
    GameEngine.v1[IO](
      gameDataWithBlankPlayer(startingAt = Coordinate(x = 2, y = 3))
        ...
        for {
          ...
          playerData <- engine.getGameData.map(_.player)
        } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 1, y = 3), history = List(PlayerInput.MoveLeft)))
      }
  }
  ...
  test("game Engine should ignore MoveUp if already at max Y".tag(Scenarios.EdgeCase)) {
    GameEngine.v1[IO](
      ...
      for {
        _ <- engine.process(PlayerInput.MoveUp)
        playerData <- engine.getGameData.map(_.player)
      } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 2, y = 5), history = List(PlayerInput.MoveUp)))
    }
  }
    ...
    GameEngine.v1[IO](
      gameDataWithBlankPlayer(startingAt = Coordinate(x = 2, y = 0))
        ...
        for {
          _ <- engine.process(PlayerInput.MoveDown)
          playerData <- engine.getGameData.map(_.player)
        } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 2, y = 0), history = List(PlayerInput.MoveDown)))
      }
  }
    ...
    GameEngine.v1[IO](
      gameDataWithBlankPlayer(startingAt = Coordinate(x = 5, y = 3))
        ...
        for {
          _ <- engine.process(PlayerInput.MoveRight)
          playerData <- engine.getGameData.map(_.player)
        } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 5, y = 3), history = List(PlayerInput.MoveRight)))
      }
  }
    ...
    GameEngine.v1[IO](
      gameDataWithBlankPlayer(startingAt = Coordinate(x = 0, y = 3))
        ...
        for {
          _ <- engine.process(PlayerInput.MoveLeft)
          playerData <- engine.getGameData.map(_.player)
        } yield assertEquals(playerData, PlayerData(coordinate = Coordinate(x = 0, y = 3), history = List(PlayerInput.MoveLeft)))
      }
  }
  ...
  test("game engine should process a generic MoveUp input successfully, for a simulated user.".tag(Scenarios.SimulatedQA)){
  ...
  test("game engine should process a generic MoveDown input successfully, for a simulated user.".tag(Scenarios.SimulatedQA)){
    PropF.forAllF(gameDataResourceArbitrary[IO].arbitrary){ gameData =>
      gameData.flatMap(GameEngine.v1[IO]).use { engine =>
        for {
          gameData <- engine.getGameData
  ...
  test("game engine should process a generic MoveRight input successfully, for a simulated user.".tag(Scenarios.SimulatedQA)){
    PropF.forAllF(gameDataResourceArbitrary[IO].arbitrary){ gameData =>
      gameData.flatMap(GameEngine.v1[IO]).use { engine =>
        for {
          gameData <- engine.getGameData
  ...
  test("game engine should process a generic MoveLeft input successfully, for a simulated user.".tag(Scenarios.SimulatedQA)){
    PropF.forAllF(gameDataResourceArbitrary[IO].arbitrary){ gameData =>
      gameData.flatMap(GameEngine.v1[IO]).use { engine =>
        for {
          gameData <- engine.getGameData
